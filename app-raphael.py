...
3. Evaluate each sections using criterias from the criteria list and state non-compliant examples in the section.
4. Suggest corrections from each non-compliant examples in the section.
...
default_checks = [
    ...
    "For every table in the document that has cost, check that the amounts are explicitly in two decimal places",
    "Look at the Annex section, check that there are sub-headings for annexes such as 'Annex 1.0' or 'Annex 1.1'",
    "Check the entire document before the Annex, check if there are any references to any non-existent sections, headers or sub-headers",
    "If the document title does not contain the word supplementary, check that the following sections exist: Annex",
